...
1. Configure a role
   ...
   To configure a role that will generate an API key and HMAC key on a Cloud Object Storage instance with policies determined by existing access groups:
   ```shell script
   $ vault write ibmcloud/roles/myRole cos_instance_guid=4f42428bc-0dca-4c42-a9fd-c8b642b0da81 access_group_ids=AccessGroupId-43f12338-fc2c-41cd-b4f9-14eff0cbeb47,AccessGroupId-43f12111-fc2c-41cd-b4f9-14eff0cbeb21
   Success! Data written to: ibmcloud/roles/myRole
   ```

   **There is a limit of 10 access groups per role.**

2. (Optional) Rotate the configured API key
...
If the role was configured with a Cloud Object Storage instance and access groups, a service credential is created for the Cloud Object Storage and its sservice ID is added as a member to all the role's access groups. The service credential's API key, HMAC access key ID and secret key are returned.
...
access groups. If the role used for key generation was configured with a Cloud Object Storage instance, the service credential is removed along with its service ID, thus removing the keys and the service ID's membership in the access groups.

### Do Not Modify Vault-owned service ID or API keys
While Vault will create and manage service IDs, service credentials, associated API keys, and access group membership, it is possible that
...
Vault generated service IDs, service credentials, and API keys will have names in this format:
...
Configures the credentials and parameters required for the plugin to perform API calls to IBM Cloud. These
credentials will be used to manage the service IDs, the access group membership, and the service credentials. The API key provided needs
...
Configures the secret engine. Must done before credentials can be generated.
...
* `resource_controller_endpoint (string: <optional>)` - The custom or private Resource Controller endpoint. For example `https://private.resource-controller.cloud.ibm.com`.
If unspecified the public endpoint, `https://resource-controller.cloud.ibm.com`, is used.
...
existing access groups. Roles may also be configured with one or more access groups and a Cloud Object Storage instance.
...
* `cos_instance_guid (string: "")` - A GUID of a Cloud Object Storage instance that will be used for generating HMAC keys. If `cos_instance_guid` is specified, `access_group_ids` must also be specified and `service_id` must not be specified.
...
Specifying a Cloud Object Storage instance with access groups:
```json
{
  "cos_instance_guid": "4f42428bc-0dca-4c42-a9fd-c8b642b0da81",
  "access_group_ids": ["AccessGroupId-43f12338-fc2c-41cd-b4f9-14eff0cbeb47", "AccessGroupId-43f12111-fc2c-41cd-b4f9-14eff0cbeb21"],
}
```
...
## Generate Secret (API key and optionally HMAC key)
...
the service ID. If the named role was also configured with a Cloud Object Storage instance GUID, service credentials are created in the instance and the HMAC access key ID and secret access keys are also generated.

The keys are renewable and have a TTL/max TLL as defined by either the role or the system default if
...
### Sample Response for a secret created from a role with a service ID or a set of access groups
```json
{
...
### Sample Response for a secret created from a role with an IBM Cloud Object Storage instance and access groups
```json
{
  "request_id": "bed3fd28-5d59-0812-4f6e-367f17567e4a",
  "lease_id": "ibmcloud/creds/dev-role/XsV7elwMQDP2j7NMEcF6EZkD",
  "renewable": true,
  "lease_duration": 2764800,
  "data": {
    "access_key_id": "7b42...",
    "api_key": "fB1MPLzVKWd...",
    "secret_access_key": "db1fd3..."
  },
  "...": "..."
}
```
## Revoking/Renewing Secrets
See Vault docs on how to renew and revoke leases.
